...
const SideNav = ({ showSideNav, width, handleClick }) => {
  return (
    ...
    <div className={`${width < 1024 ? 'visible' : 'hidden'} ${showSideNav ? '-translate-x-0' : 'translate-x-full'} transform top-0 right-0 w-halfScreen h-full bg-gray-100 overflow-auto ease-in-out fixed transition-all duration-500 flex flex-col space-y-10 justify-center p-6 lg:overflow-visible lg:transition-none lg:bg-transparent lg:h-auto lg:relative lg:flex-row lg:flex-grow lg:items-center lg:w-auto lg:space-y-0 lg:py-10 lg:ml-32 lg:mr-8 shadow-xl`}>
      ...
      <div className="flex flex-col space-y-10 lg:space-y-0 lg:flex-row items-center flex-wrap lg:flex">
       <a href="#resume" className={`text-xl px-8 py-4 lg:text-sm lg:px-4 lg:py-2 lg:mr-4 ${buttonStyles}`} onClick={handleClick}>Resume</a>
